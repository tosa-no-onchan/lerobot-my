...
# 🌟 Hugging FaceのPEFT（LoRA用ライブラリ）をインポート
...
class DiffCarPolicy(nn.Module):
    def __init__(self, obs_horizon=2, pred_horizon=16):
        ...
        # 1. まずは通常通り DINOv2 (ViT-S/14) をロード
        ...
        # 🌟 2. LoRA の設定（チューニングの味付け）を定義
        lora_config = LoraConfig(
            r=8,                # LoRAのランク（4〜16が一般的。小さいほど軽量で過学習しにくい）
            lora_alpha=16,      # スケーリング因子（通常は r の2倍に設定）
            # どの層にLoRAを挟むか指定。ViTの場合はQuery(q)とValue(v)のプロジェクション層が鉄板
            #target_modules=["q_proj", "v_proj"], 
            # 🌟 ここを修正します！ 🌟
            # PyTorch Hub版 DINOv2 のアテンション層の正しいターゲット名です
            target_modules=["qkv", "proj"], 

            lora_dropout=0.05,  # 過学習を防ぐためのドロップアウト率
            bias="none"
        )

        # 🌟 3. DINOv2をLoRAモデルに一瞬で変換！
        # これにより、DINOv2本体は自動で【完全固定】され、
        # target_modulesで指定した場所にだけ「学習可能な小さな層」が追加されます。
        self.vision_encoder = get_peft_model(base_model, lora_config)

        # 🌟 4. デバッグ表示（どれだけ節約できたか確認できます）
